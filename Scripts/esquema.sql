...
CREATE TABLE paciente (
    ...
    CONSTRAINT ck_paciente_celular CHECK (celular IS NULL OR celular ~ E'^\\(\\d{2}\\)\\d{9}$'),
    CONSTRAINT ck_paciente_fixo CHECK (fixo IS NULL OR fixo ~ E'^\\(\\d{2}\\)\\d{8}$'),
    CONSTRAINT ck_paciente_email CHECK (email IS NULL OR email ~ E'^[A-Za-z0-9._%+-]+@email\\.com$')
);
...
CREATE TABLE medico (
    ...
    CONSTRAINT ck_medico_celular CHECK (celular IS NULL OR celular ~ E'^\\(\\d{2}\\)\\d{9}$'),
    CONSTRAINT ck_medico_fixo CHECK (fixo IS NULL OR fixo ~ E'^\\(\\d{2}\\)\\d{8}$'),
    CONSTRAINT ck_medico_email CHECK (email IS NULL OR email ~ E'^[A-Za-z0-9._%+-]+@email\\.com$')
);
...
CREATE TABLE convenio (
    ...
    CONSTRAINT ck_convenio_desconto CHECK (desconto >= 0 AND desconto <= 1) 
);
...
CREATE TABLE unidade ( 
    ...
    CONSTRAINT ck_unidade_telefone CHECK (telefone IS NULL OR telefone ~ E'^\\(\\d{2}\\)\\d{8}$'),
...
CREATE TABLE funcionamento (
    ...
    CONSTRAINT ck_dia CHECK (dia IN ('Segunda','Terca','Quarta','Quinta','Sexta','Sabado','Domingo')),
